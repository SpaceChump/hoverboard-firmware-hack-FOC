...
#define BAT_CALIB_REAL_VOLTAGE  4080      // input voltage measured by multimeter (multiplied by 100). In this case 43.00 V * 100 = 4300
#define BAT_CALIB_ADC           1624      // adc-value measured by mainboard (value nr 5 on UART debug output)
...
#define BAT_LVL5                (380 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Green blink:  no beep
#define BAT_LVL4                (360 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Yellow:       no beep
#define BAT_LVL3                (345 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Yellow blink: no beep 
...
#define BAT_LVL1                (310 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Red blink:    fast beep. Your battery is almost empty. Charge now! [V*100/cell]. In this case 3.50 V/cell
#define BAT_DEAD                (290 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // All leds off: undervoltage poweroff. (while not driving) [V*100/cell]. In this case 3.37 V/cell
// ######################## END OF BATTERY ###############################
...
#define I_DC_MAX        28               // [A] Maximum stage2 DC Link current limit for Commutation and Sinusoidal types (This is the final current protection. Above this value, current chopping is applied. To avoid this make sure that I_DC_MAX = I_MOT_MAX + 2A)
...
// Communication:         [DONE]
// Balancing controller:  [DONE]  (Implemented on sideboard)
#ifdef VARIANT_ONEWHEEL
  #define FLASH_WRITE_KEY     0x1011          // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
  ...
  #define FEEDBACK_SERIAL_USART2
  //#define SIDEBOARD_SERIAL_USART3 1           // right sensor board cable. Number indicates priority for dual-input. Disable if I2C (nunchuk or lcd) is used!
  //#define FEEDBACK_SERIAL_USART3
  ...
  #define RATE                  32767         // Instant acceleration rate
  #define FILTER                65535         // No low-pass filtering (1.0 multiplier)
...
// ############################### VALIDATE SETTINGS ###############################
    ...
    !defined(VARIANT_IBUS) && !defined(VARIANT_HOVERCAR) && !defined(VARIANT_HOVERBOARD) && !defined(VARIANT_TRANSPOTTER) && !defined(VARIANT_SKATEBOARD) && !defined(VARIANT_ONEWHEEL)
